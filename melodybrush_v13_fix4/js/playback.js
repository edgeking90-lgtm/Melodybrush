...
function startPlay() {
  ...
  _playing = true;
  // PC/모바일 공통 버튼 업데이트
  const _playIcon = document.getElementById('play-icon');
  const _playBtn  = document.getElementById('btn-play');
  if (_playIcon) { _playIcon.textContent = '■'; const lbl = document.getElementById('play-lbl'); if (lbl) lbl.textContent = '정지'; }
  else if (_playBtn) _playBtn.innerHTML = '■<span class="lbl">Stop</span>';
...
function stopPlay() {
  ...
  // PC/모바일 공통 버튼 복구
  const _stopIcon = document.getElementById('play-icon');
  const _stopBtn  = document.getElementById('btn-play');
  if (_stopIcon) { _stopIcon.textContent = '▶'; const lbl = document.getElementById('play-lbl'); if (lbl) lbl.textContent = '재생'; }
  else if (_stopBtn) _stopBtn.innerHTML = '▶<span class="lbl">Showing</span>';
  render();
}
