function parseMidi(buf) {
  ...
  for (let t = 0; t < nTracks; t++) {
    ...
    while (pos < end) {
      ...
      // [FIX] meta event는 status===0xff로 직접 비교
      if (status === 0xff) {
...
function runExpressionEngine() {
  ...
  const events = [], vs = +document.getElementById('velScale') ? +document.getElementById('velScale').value : 1.0;
  const pbSemi = 2, pbScale = 8191 / pbSemi;
  midiTracks.forEach((tr, trIdx) => {
    ...
    tr.notes.forEach(n => {
      ...
      cells.forEach((c, i) => {
        ...
        const cc11 = Math.round(clamp(100 * Math.pow(1 - r, 1.5), 5, 100));
