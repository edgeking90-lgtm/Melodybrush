...
function render() {
  ...
  if (midiLines.length) {
    const lod = getLOD();
    // Core line width scales with zoom — thinner as we zoom in
    const coreW = [6, 5, 3.5, 2.5, 2][lod];

    midiLines.forEach(l => {
      const isSel = selectedLine === l, dimmed = selectedLine && !isSel;
      const baseA = dimmed ? 0.12 : 1.0;

      if (appMode === 'view') {
        // LOD 0-1: only smooth coreline (Forest/Line)
        if (lod <= 1) {
          drawSmoothCoreline(l.smoothCoreline, l.color, coreW + (isSel ? 1.5 : 0), baseA * (isSel ? 1.0 : 0.85));
        }
        // LOD 2: Stroke segments start appearing
        else if (lod === 2) {
          drawFiberGesture(l, baseA * 0.25);
          drawSmoothCoreline(l.smoothCoreline, l.color, coreW + (isSel ? 1.5 : 0), baseA * 0.82);
        }
        // LOD 3: Cell dots + fiber
        else if (lod === 3) {
          drawFiberGesture(l, baseA * 0.55);
          drawCellDots(l, baseA * 0.5);
          drawSmoothCoreline(l.smoothCoreline, l.color, coreW + (isSel ? 1 : 0), baseA * 0.55);
        }
        // LOD 4: Gesture detail, coreline fades
        else {
          drawFiberGesture(l, baseA * 0.85);
          drawCellDots(l, baseA * 0.75);
          drawSmoothCoreline(l.smoothCoreline, l.color, coreW + (isSel ? 1 : 0), baseA * 0.32);
        }
      } else if (appMode === 'note') {
        if (lod >= 2) drawFiberGesture(l, baseA * (lod <= 2 ? 0.4 : 0.65));
        if (lod >= 3) drawCellDots(l, baseA * 0.6);
        const coreAlpha = lod >= 3 ? 0.42 : 0.82;
        drawSmoothCoreline(l.smoothCoreline, l.color, coreW + (isSel ? 1.5 : 0), baseA * coreAlpha);
      } else {
