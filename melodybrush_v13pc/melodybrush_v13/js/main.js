...
cv.addEventListener('wheel', e => {
  ...
  if (e.shiftKey) {
    ...
    // Alt+wheel: vertical zoom only
    vpZoomAt(x, y, 1, 1, inward);
  } else {
    // Normal: uniform zoom, slightly favour horizontal (x1.0 vs y0.85) for line visibility
...
document.addEventListener('keydown', e => {
  ...
  switch (e.key) {
    ...
    case '+': case '=': vpZoomAt(CW / 2, CH / 2, 1, 0.82, 0.88); refreshLegend(); render(); break;
    case '-': vpZoomAt(CW / 2, CH / 2, 1, 1.22, 1.14); refreshLegend(); render(); break;
    case '>': case '.': vpZoomAt(CW / 2, CH / 2, 1, 0.80, 1); refreshLegend(); render(); break;  // H zoom in
    case '<': case ',': vpZoomAt(CW / 2, CH / 2, 1, 1.25, 1); refreshLegend(); render(); break;  // H zoom out
  }
});
